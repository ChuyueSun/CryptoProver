...
**Sibling integrity gates.** Four more gates guard the same "the agent's incentive is to fake a green" threat, each snapshotted before the loop and diffed after every round (drift → break + a non-promotable `end_reason`): **axiom** (`AXIOM_DRIFT` — new `axiom_*`), **tooling** (`TOOLING_DRIFT` — any edit to top-level `*.py`/`*.sh`, prompt files, or executable/policy files recursively under `skills/`, `lib/`, `docker/`, and `scripts/`; checked first, since doctored tooling makes the others untrustworthy), **forbidden-construct** (`FORBIDDEN_CONSTRUCT` — any *increase* in `assume(...)`/`external_body`, runs even when the spec gate is off), and the **spec-definition freeze** (`--check-spec-defs`, on whenever the spec gate is on — freezes `spec fn` *bodies*, not just headers, so a definition can't be redefined to hollow a frozen contract). The frozen-file guard includes untracked files in every run mode; newly created helpers fail closed. Source receipts close over literal Rust `include!`/`include_str!`/`include_bytes!` and `#[path]` dependencies below generated-directory exclusions, and any such off-walk input that appears or changes after baseline is `FROZEN_EDIT`. See CLAUDE.md → **Spec integrity gate** for the full text. Don't relax these.
