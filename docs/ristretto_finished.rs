...
impl CompressedRistretto {
            ...
            result.is_some() ==> is_in_even_subgroup(result.unwrap().0),
    {
        ...
        if choice_into(choice_or(choice_not(s_encoding_is_canonical), s_is_negative)) {
            proof {
                // We are in the None branch.  Show spec_ristretto_decompress returns None.
                // Either s_is_negative ==> is_negative(field_element_from_bytes(&self.0))
                // or  !s_encoding_is_canonical ==> u8_32_as_nat(&self.0) >= p().
                if !choice_is_true(s_encoding_is_canonical) {
                    if u8_32_as_nat(&self.0) < p() {
                        lemma_canonical_check_backward(&self.0, &s);
                        // contradicts !choice_is_true(s_encoding_is_canonical)
                        assert(spec_fe51_as_bytes(&s) == self.0@);
                        assert(false);
                    }
                }
            }
        ...
        // After the canonical && !negative checks pass:
        //   - choice_is_true(s_encoding_is_canonical) ==> u8_32_as_nat(&self.0) < p() (from step_1)
        //   - !choice_is_true(s_is_negative) ==> !is_negative(field_element_from_bytes(&self.0))
        //   - fe51_as_canonical_nat(&s) == field_element_from_bytes(&self.0)
        proof {
            // No further facts needed — Verus has all of the above from step_1 ensures.
        }
        ...
        if choice_into(choice_or(choice_or(choice_not(ok), t_is_negative), y_is_zero)) {
            let result = None;
            proof {
                // Spec returns None when !ristretto_decode_ok(s_spec)
                // or is_negative(t_spec) or y_spec == 0.
                // step_2 ensures map each exec flag to the matching spec condition.
            }
            result
        } else {
            let result = Some(res);
            proof {
                // Spec returns Some((decode_x, decode_y, 1, field_mul(decode_x, decode_y))).
                // step_2 ensures (when ok): X_nat == decode_x, Y_nat == decode_y,
                // Z_nat == 1, T_nat == field_mul(X_nat, Y_nat), plus well-formed and even-subgroup.
...
mod decompress {
            ...
            choice_is_true(result.0) ==> u8_32_as_nat(&repr.0) < p(),
    {
        ...
        proof {
            // Bridge as_bytes (exec) to spec_fe51_as_bytes:
            //   u8_32_as_nat(&s_bytes_check) == fe51_as_canonical_nat(&s)
            //                                == u64_5_as_nat(s.limbs) % p()
            lemma_as_bytes_equals_spec_fe51_to_bytes(&s, &s_bytes_check);
            assert(spec_fe51_as_bytes(&s) == s_bytes_check@);

            // (3) s_encoding_is_canonical <==> spec_fe51_as_bytes(&s) == repr.0@.
            //   exec: choice == (s_bytes_check == repr.0)
            //   For [u8;32], array equality and Seq equality coincide.
            assert((s_bytes_check == repr.0) <==> (s_bytes_check@ == repr.0@)) by {
                if s_bytes_check@ == repr.0@ {
                    lemma_seq_eq_implies_array_eq(&s_bytes_check, &repr.0);
                }
            };

            // Sign-bit identity: spec_fe51_as_bytes(s)[0]&1 == fe51_as_canonical_nat(s) % 2.
            lemma_is_negative_equals_parity(&s);

            // (5) Reconcile fe51_as_canonical_nat(&s) with field_element_from_bytes(&repr.0):
            //     both equal (u8_32_as_nat(&repr.0) % pow2(255)) % p().
            //     Since field_element_from_bytes(b) < p, field_canonical applied to it is identity.
            assert(fe51_as_canonical_nat(&s) == field_element_from_bytes(&repr.0));
            pow255_gt_19();
            let m: nat = (u8_32_as_nat(&repr.0) % pow2(255)) as nat;
            assert(p() > 0);
            vstd::arithmetic::div_mod::lemma_mod_decreases(m, p());
            assert(m % p() < p());
            assert(field_element_from_bytes(&repr.0) == m % p());
            assert(field_element_from_bytes(&repr.0) < p());
            vstd::arithmetic::div_mod::lemma_small_mod(
                field_element_from_bytes(&repr.0),
                p(),
            );
            assert(is_negative(field_element_from_bytes(&repr.0)) == (
                field_element_from_bytes(&repr.0) % 2 == 1));
            assert(is_negative(field_element_from_bytes(&repr.0)) == (
                fe51_as_canonical_nat(&s) % 2 == 1));

            // (6) Forward canonical:  s_encoding_is_canonical ==> u8_32_as_nat(&repr.0) < p().
            if choice_is_true(s_encoding_is_canonical) {
                assert(s_bytes_check == repr.0);
                // u8_32_as_nat(&s_bytes_check) == fe51_as_canonical_nat(&s) < p (always).
                assert(u8_32_as_nat(&s_bytes_check) == fe51_as_canonical_nat(&s));
                assert(fe51_as_canonical_nat(&s) < p());
                assert(u8_32_as_nat(&repr.0) == u8_32_as_nat(&s_bytes_check));
            }
        }
            ...
            choice_is_true(result.2) == (ristretto_decode_y(fe51_as_canonical_nat(&s)) == 0),
    {
        ...
        // =================================================================
        proof {
            crate::lemmas::field_lemmas::constants_lemmas::lemma_one_limbs_bounded_51();
            crate::lemmas::edwards_lemmas::constants_lemmas::lemma_edwards_d_limbs_bounded_54();
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&s, 51, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&FieldElement::ONE, 51, 52);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&FieldElement::ONE, 51, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&one, &ss, 52);
        }
        let u2 = &one + &ss;  //  1 - as²    where a=-1
        // u2: 53-bit (Add with 52+52 inputs → 53-bit via 2nd branch)

        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&u2, 53, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&neg_d, 52, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&I, 52, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&s, &s, 51);
        }
        let s_plus_s = &s + &s;

        // x == | 2s/sqrt(v) | == + sqrt(4s²/(ad(1+as²)² - (1-as²)²))
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&s_plus_s, 52, 54);
        }
        ...
        */
        proof {
            axiom_step2_constructed_point_well_formed(x, y, one, t, fe51_as_canonical_nat(&s));
        }
        ...
        proof {
            ...
            axiom_step2_postconditions(
                s,
                choice_is_true(ok),
                choice_is_true(t_is_neg),
                choice_is_true(y_is_zero),
                point,
            );
        }
...
impl RistrettoPoint {
            ...
            result.0 == spec_ristretto_compress(*self),
    {
        ...
        proof {
            crate::specs::edwards_specs::lemma_unfold_edwards(self.0);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.X, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.Y, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.Z, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.T, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&self.0.Z, &self.0.Y, 54);
            lemma_sqrt_m1_limbs_bounded();
            crate::lemmas::field_lemmas::constants_lemmas::lemma_invsqrt_a_minus_d_limbs_bounded();
        }
        ...
        let z_plus_y = Z + &Y;
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&z_plus_y, 53, 54);
        }
        let z_minus_y = Z - &Y;
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&invsqrt, 52, 54);
        }
        ...
        proof {
            crate::lemmas::ristretto_lemmas::axioms::axiom_ristretto_compress_correct(*self, s_bytes);
        }
...
impl<'a> From<&'a RistrettoPoint> for BatchCompressState {
            ...
            }),
    {
        proof {
            ...
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&P.0.X, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&P.0.Y, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&P.0.Z, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&P.0.T, 52, 54);
            crate::lemmas::field_lemmas::constants_lemmas::lemma_elligator_constants_bounded();
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::EDWARDS_D, 51, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&P.0.Y, &P.0.Y, 54);
        }
        let Y2 = &P.0.Y + &P.0.Y;  // = 2*Y
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&Y2, 53, 54);
        }

        let e = &P.0.X * &Y2;  // = 2*X*Y
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&ZZ, &dTT, 54);
        }
        let f = &ZZ + &dTT;  // = Z^2 + d*T^2
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&YY, &XX, 54);
        }
        let g = &YY + &XX;  // = Y^2 - a*X^2
        let h = &ZZ - &dTT;  // = Z^2 - d*T^2

        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&f, 53, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&g, 53, 54);
        }
        let eg = &e * &g;
        let fh = &f * &h;

        proof {
            // Bridge closed accessors edwards_x/y/z/t to actual fields P.0.X/Y/Z/T.
            crate::specs::edwards_specs::lemma_unfold_edwards(P.0);
            ...
            let x_raw = crate::specs::field_specs_u64::u64_5_as_nat(P.0.X.limbs);
            let y_raw = crate::specs::field_specs_u64::u64_5_as_nat(P.0.Y.limbs);
            let z_raw = crate::specs::field_specs_u64::u64_5_as_nat(P.0.Z.limbs);
            let t_raw = crate::specs::field_specs_u64::u64_5_as_nat(P.0.T.limbs);
            let xx_raw = crate::specs::field_specs_u64::u64_5_as_nat(XX.limbs);
            let yy_raw = crate::specs::field_specs_u64::u64_5_as_nat(YY.limbs);
            let zz_raw = crate::specs::field_specs_u64::u64_5_as_nat(ZZ.limbs);
            let tt_raw = crate::specs::field_specs_u64::u64_5_as_nat(TT.limbs);

            // Bridge square exec → field_square spec.
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_square_matches_field_square(x_raw, xx_raw);
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_square_matches_field_square(y_raw, yy_raw);
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_square_matches_field_square(z_raw, zz_raw);
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_square_matches_field_square(t_raw, tt_raw);

            assert(fe51_as_canonical_nat(&XX) == field_square(x));
            assert(fe51_as_canonical_nat(&YY) == field_square(y));
            assert(fe51_as_canonical_nat(&ZZ) == field_square(z));
            assert(fe51_as_canonical_nat(&TT) == field_square(t));

            // e = X * (Y+Y) = X * (2Y) = 2 * X*Y in the field.
            assert(fe51_as_canonical_nat(&Y2) == field_add(y, y));
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_add_self_eq_double(y);
            assert(fe51_as_canonical_nat(&Y2) == field_mul(2, y));
            assert(fe51_as_canonical_nat(&e) == field_mul(x, field_mul(2, y)));
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_mul_assoc(x, 2, y);
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_mul_comm(x, 2);
            assert(field_mul(x, 2) == field_mul(2, x));
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_mul_assoc(2, x, y);
            assert(fe51_as_canonical_nat(&e) == field_mul(2, field_mul(x, y)));

            // f = ZZ + dTT, where dTT = TT * d = field_square(t) * d.
            assert(fe51_as_canonical_nat(&dTT) == field_mul(field_square(t), d));
            crate::lemmas::field_lemmas::field_algebra_lemmas::lemma_field_mul_comm(field_square(t), d);
            assert(fe51_as_canonical_nat(&dTT) == field_mul(d, field_square(t)));
            assert(fe51_as_canonical_nat(&f) == field_add(field_square(z), field_mul(d, field_square(t))));

            // g = YY + XX = field_square(y) + field_square(x)
            assert(fe51_as_canonical_nat(&g) == field_add(field_square(y), field_square(x)));

            // h = ZZ - dTT = field_square(z) - field_mul(d, field_square(t))
            assert(fe51_as_canonical_nat(&h) == field_sub(field_square(z), field_mul(d, field_square(t))));

            // eg = e * g, fh = f * h
            assert(fe51_as_canonical_nat(&eg) == field_mul(fe51_as_canonical_nat(&e), fe51_as_canonical_nat(&g)));
            assert(fe51_as_canonical_nat(&fh) == field_mul(fe51_as_canonical_nat(&f), fe51_as_canonical_nat(&h)));
        }
...
impl RistrettoPoint {
                ...
                },
    {
        ...
        proof {
            crate::lemmas::field_lemmas::constants_lemmas::lemma_elligator_constants_bounded();
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::INVSQRT_A_MINUS_D, 51, 54);
        }
            ...
            decreases states.len() - j,
        {
            ...
            proof {
                crate::lemmas::ristretto_lemmas::axioms::axiom_batch_loop_iteration_correct(
                    compressed, points[j as int],
                );
            }
            ...
            is_ristretto_coset(result, self.0),
    {
        ...
        proof {
            ...
            crate::specs::edwards_specs::lemma_unfold_edwards(t2);
            crate::specs::edwards_specs::lemma_unfold_edwards(t4);
            crate::specs::edwards_specs::lemma_unfold_edwards(t6);
        ...
        proof {
            ...
            assert(result[2] == p2);
            assert(result[3] == p3);
            assert(t2 == crate::backend::serial::u64::constants::spec_eight_torsion()[2]);
            assert(t4 == crate::backend::serial::u64::constants::spec_eight_torsion()[4]);
            assert(t6 == crate::backend::serial::u64::constants::spec_eight_torsion()[6]);
        }
        result
    }
            ...
            is_in_even_subgroup(result.0),
    {
        ...
        proof {
            ...
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&FieldElement::ONE, 51, 52);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&FieldElement::ONE, 51, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::MINUS_ONE, 51, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::EDWARDS_D, 51, 52);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::ONE_MINUS_EDWARDS_D_SQUARED, 51, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::EDWARDS_D_MINUS_ONE_SQUARED, 51, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&constants::SQRT_AD_MINUS_ONE, 51, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&r, &one, 54);
        }
        let r_plus_one = &r + &one;
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&r_plus_one, 53, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&r, d, 54);
        }
        let r_plus_d = &r + d;
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&r_plus_d, 53, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&s, 52, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&s, &s, 54);
        }
        let s_plus_s = &s + &s;
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&s_plus_s, 53, 54);
        }
        let cp_X = &s_plus_s * &D;
        ...
        proof {
            crate::lemmas::field_lemmas::add_lemmas::lemma_sum_of_limbs_bounded_from_fe51_bounded(&FieldElement::ONE, &s_sq, 54);
        }
        ...
        proof {
            axiom_elligator_cp_valid(r_0, cp);
        }
        ...
        proof {
            axiom_elligator_postconditions(r_0_nat, extended);
        }
            ...
            is_uniform_bytes(bytes) ==> is_uniform_ristretto_point(&result),
    {
        ...
        proof {
            // from_bytes ensures fe51_limbs_bounded(&r_1, 51); relax to 54.
            assert((1u64 << 51) <= (1u64 << 54)) by (bit_vector);
            assert(fe51_limbs_bounded(&r_1, 54));
        }
        ...
        proof {
            assert((1u64 << 51) <= (1u64 << 54)) by (bit_vector);
            assert(fe51_limbs_bounded(&r_2, 54));
        }
        ...
        let result = R_1 + R_2;
        proof {
            // Even subgroup is closed under +.
            crate::lemmas::ristretto_lemmas::axioms::axiom_even_subgroup_closed_under_add(R_1.0, R_2.0);

            // Bridge bytes views to spec choose-values.
            let b1 = crate::specs::ristretto_specs::uniform_bytes_first(bytes);
            let b2 = crate::specs::ristretto_specs::uniform_bytes_second(bytes);
            assert(b1@ == bytes@.subrange(0, 32));
            assert(b2@ == bytes@.subrange(32, 64));
            assert(r_1_bytes@ == b1@);
            assert(r_2_bytes@ == b2@);
            crate::lemmas::ristretto_lemmas::axioms::lemma_u8_32_as_nat_view_eq(&r_1_bytes, &b1);
            crate::lemmas::ristretto_lemmas::axioms::lemma_u8_32_as_nat_view_eq(&r_2_bytes, &b2);
            assert(field_element_from_bytes(&r_1_bytes) == field_element_from_bytes(&b1));
            assert(field_element_from_bytes(&r_2_bytes) == field_element_from_bytes(&b2));
            assert(fe51_as_canonical_nat(&r_1) == field_element_from_bytes(&r_1_bytes));
            assert(fe51_as_canonical_nat(&r_2) == field_element_from_bytes(&r_2_bytes));

            // Uniformity branch: if input bytes are uniform, the result is uniform.
            if is_uniform_bytes(bytes) {
                crate::specs::proba_specs::axiom_uniform_bytes_split(bytes, &r_1_bytes, &r_2_bytes);
                assert(fe51_as_nat(&r_1) == u8_32_as_nat(&r_1_bytes) % vstd::arithmetic::power2::pow2(255));
                assert(fe51_as_nat(&r_2) == u8_32_as_nat(&r_2_bytes) % vstd::arithmetic::power2::pow2(255));
                crate::specs::proba_specs::axiom_from_bytes_uniform(&r_1_bytes, &r_1);
                crate::specs::proba_specs::axiom_from_bytes_uniform(&r_2_bytes, &r_2);
                crate::specs::proba_specs::axiom_from_bytes_independent(&r_1_bytes, &r_2_bytes, &r_1, &r_2);
                crate::specs::proba_specs::axiom_uniform_elligator(&r_1, &R_1);
                crate::specs::proba_specs::axiom_uniform_elligator(&r_2, &R_2);
                crate::specs::proba_specs::axiom_uniform_elligator_independent(&r_1, &r_2, &R_1, &R_2);
                crate::specs::proba_specs::axiom_uniform_elligator_sum(&R_1, &R_2, &result);
            }
        }
        result
    }
...
impl Identity for RistrettoPoint {
            ...
            is_in_even_subgroup(result.0),
    {
        ...
        proof {
            // ep's affine representation is the identity (0, 1).
            // Witness q = ep: edwards_scalar_mul((0, 1), 2) == (0, 1).
            lemma_edwards_scalar_mul_identity(2);
            assert(edwards_point_as_affine(ep) == edwards_identity());
            assert(edwards_scalar_mul(edwards_identity(), 2) == edwards_identity());
            assert(edwards_point_as_affine(ep) == edwards_scalar_mul(
                edwards_point_as_affine(ep),
                2,
            ));
        }
        RistrettoPoint(ep)
...
impl ConstantTimeEq for RistrettoPoint {
            ...
            choice_is_true(result) == ristretto_equivalent(self.0, other.0),
    {
        proof {
            ...
            crate::specs::edwards_specs::lemma_unfold_edwards(self.0);
            crate::specs::edwards_specs::lemma_unfold_edwards(other.0);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.X, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&self.0.Y, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&other.0.X, 52, 54);
            crate::lemmas::field_lemmas::add_lemmas::lemma_fe51_limbs_bounded_weaken(&other.0.Y, 52, 54);
        }
        ...
        proof {
            crate::lemmas::field_lemmas::as_bytes_lemmas::lemma_ct_eq_iff_canonical_nat(&X1Y2, &Y1X2);
            crate::lemmas::field_lemmas::as_bytes_lemmas::lemma_ct_eq_iff_canonical_nat(&X1X2, &Y1Y2);
            crate::lemmas::ristretto_lemmas::axioms::axiom_ristretto_cross_mul_iff_equivalent(self.0, other.0);
        }
...
impl RistrettoPoint {
            ...
            edwards_point_as_affine(result.0) == sum_of_ristretto_points(points@),
    {
        ...
        proof {
            crate::specs::edwards_specs::lemma_identity_affine_coords(acc.0);
                    ...
                    ),
        {
            ...
            let ghost prev_subrange = points@.subrange(0, i as int);
            assert(is_well_formed_edwards_point(points@[i as int].0));

            acc = &acc + &points[i];

            proof {
                let new_subrange = points@.subrange(0, (i + 1) as int);
                assert(new_subrange.len() == (i + 1) as int);
                assert(new_subrange[(i) as int] == points@[i as int]);
                assert(new_subrange.subrange(0, i as int) =~= prev_subrange);
            }
        }
...
impl<T> Sum<T> for RistrettoPoint where T: Borrow<RistrettoPoint> {
            ...
            ),
    {
        ...
        proof {
            let extracted = Seq::new(points@.len(), |i: int| points@[i].0);
            assert(extracted =~= iter_spec) by {
                assert(points@.len() == iter_spec.len());
                assert(forall|i: int| 0 <= i < points@.len() ==>
                    (#[trigger] points@[i]).0 == iter_spec[i]);
            };
            crate::specs::iterator_specs::lemma_sum_ristretto_edwards_equiv(points@);
        }
